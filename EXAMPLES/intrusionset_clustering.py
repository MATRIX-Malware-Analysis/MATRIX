...
def extract_graph(tx):
    query = """
    MATCH (i:IntrusionSet)-[:uses]->(b:MalwareBehavior)
    RETURN i.name AS intrusion_set, b.name AS behavior
    """
    result = tx.run(query)
    edges = [(record["intrusion_set"], record["behavior"]) for record in result]
    return edges

def extract_nodes(tx):
    query = """
    MATCH (i:IntrusionSet)
    RETURN i.name AS intrusion_set, i.id AS intrusion_set_id
    """
    result = tx.run(query)
    intrusion_set_nodes = {record["intrusion_set"]: record["intrusion_set_id"] for record in result}
    ...
    return intrusion_set_nodes, behavior_nodes

with driver.session() as session:
    edges = session.read_transaction(extract_graph)
    intrusion_set_nodes, behavior_nodes = session.read_transaction(extract_nodes)
...
for i, community in enumerate(communities):
    community_intrusion_sets = [node for node in community if node in intrusion_set_nodes]
    ...
    for behavior in community_behaviors:
        for neighbor in G.neighbors(behavior):
            if neighbor in community_intrusion_sets:
    ...
    community_results.append({
        "community_id": i,
        "intrusion_sets": community_intrusion_sets,
        "top_behaviors": top_behaviors
    })

# Salva i risultati in un file JSON
output_file = "IntrusionSet_community_results.json"
