from neo4j import GraphDatabase
import networkx as nx
from networkx.algorithms.community import louvain_communities
from collections import Counter
import json

# Connessione a Neo4j
uri = "bolt://localhost:7688"
user = "neo4j"
password = "malware_profiler"

driver = GraphDatabase.driver(uri, auth=(user, password))

def extract_graph(tx):
    query = """
    MATCH (d:DataComponent)-[:detects]->(b:MalwareBehavior)
    RETURN d.name AS data_component, b.name AS behavior
    """
    result = tx.run(query)
    edges = [(record["data_component"], record["behavior"]) for record in result]
    return edges

def extract_nodes(tx):
    query = """
    MATCH (d:DataComponent)
    RETURN d.name AS data_component, d.id AS data_component_id
    """
    result = tx.run(query)
    data_component_nodes = {record["data_component"]: record["data_component_id"] for record in result}
    
    query = """
    MATCH (b:MalwareBehavior)
    RETURN b.name AS behavior, b.id AS behavior_id
    """
    result = tx.run(query)
    behavior_nodes = {record["behavior"]: record["behavior_id"] for record in result}

    return data_component_nodes, behavior_nodes

with driver.session() as session:
    edges = session.read_transaction(extract_graph)
    data_component_nodes, behavior_nodes = session.read_transaction(extract_nodes)

G = nx.Graph()
G.add_edges_from(edges)

# Algoritmo di clustering Louvain
communities = louvain_communities(G)

community_results = []

for i, community in enumerate(communities):
    community_data_components = [node for node in community if node in data_component_nodes]
    community_behaviors = [node for node in community if node in behavior_nodes]
    
    behavior_counter = Counter()
    
    for behavior in community_behaviors:
        for neighbor in G.neighbors(behavior):
            if neighbor in community_data_components:
                behavior_counter[behavior] += 1
    
    top_behaviors = behavior_counter.most_common(3)
    top_behaviors = [(behavior, count) for behavior, count in top_behaviors]
    
    community_results.append({
        "community_id": i,
        "data_components": community_data_components,
        "top_behaviors": top_behaviors
    })

# Salva i risultati in un file JSON
output_file = "DataComponent_community_results.json"
with open(output_file, 'w', encoding='utf-8') as f:
    json.dump(community_results, f, indent=4)

print(f"Results saved to {output_file}")

# Chiusura della connessione a Neo4j
driver.close()
